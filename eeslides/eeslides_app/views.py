        ...
        try:
            ...
            if currdate in newfiledata:
                for p in newfiledata[currdate]:
                    ...
                    if not p["parsed"] and time.time() - p["time"] < MEMORY_DURATION:
                        p["parsed"] = True
                        pred = p['prediction']
                        if pred in timedata:
                            lastpredtime = timedata[pred]
                            # wait at least 10 seconds before adding it again
                            if time.time() - lastpredtime > WAIT_TIME:
                                names.append(pred)
            ...
            # print '\tUpdating updates_django.json took {0:.2f} seconds'.format(time.time() - now)
